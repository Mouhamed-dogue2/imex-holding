    ...
    <footer style={{ background:'var(--bg-secondary)', borderTop:'1px solid var(--border)', padding:'60px 48px 28px' }}>
      <div style={{ maxWidth:'1200px', margin:'0 auto' }}>
        <div style={{ display:'grid', gridTemplateColumns:'2fr 1fr 1fr 1fr', gap:'48px', marginBottom:'48px' }}>
          ...
          {/* Brand */}
          <div>
            <button onClick={() => navigate('/')} style={{ background:'none', border:'none', cursor:'pointer', padding:0, marginBottom:'18px', display:'block' }}>
              <div style={{ height:'44px', padding:'4px 10px', background:'#fff', borderRadius:'7px', display:'inline-flex', alignItems:'center', boxShadow:'0 2px 10px rgba(0,0,0,0.12)', transition:'all 0.25s' }}
              ...
              >
                <img src="/images/logo.png" alt="IMEX HOLDING" style={{ height:'34px', width:'auto', objectFit:'contain' }}/>
              </div>
            </button>
            <p style={{ fontSize:'13.5px', color:'var(--text-muted)', lineHeight:1.85, maxWidth:'260px', fontWeight:300, marginBottom:'16px' }}>{f.desc}</p>
          ...
          {[
            ...
            <div key={ci}>
              <h4 style={{ fontSize:'10.5px', letterSpacing:'2.5px', textTransform:'uppercase', color:'var(--text-muted)', marginBottom:'18px', fontWeight:500 }}>{col.title}</h4>
              <ul style={{ listStyle:'none' }}>
                {col.items.map((item,ii) => (
                  <li key={ii} style={{ marginBottom:'11px' }}>
                    <button onClick={() => col.routes[ii]!=='#' && navigate(col.routes[ii])}
                      style={{ background:'none', border:'none', cursor:col.routes[ii]!=='#'?'pointer':'default', fontSize:'13.5px', color:'var(--text-muted)', fontFamily:'Inter,sans-serif', fontWeight:300, padding:0, textAlign:'left', transition:'color 0.2s' }}
